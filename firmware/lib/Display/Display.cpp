...
namespace com::ironbird::esp32dabradio {
    void prefixAndSuffixStringWithSpaces(const char source[], char destination[], const uint8_t prefixLength,
                                         const uint8_t suffixLength) {
    ...
    void Display::rollLeft(const char source[], char destination[], uint8_t size, long cycle) {
        ...
        unsigned int apparentLength = sourceLength + size - 2;
        // Length of string that would roll if it was one (we want the first character to reappear when the last one is displayed)
        unsigned int sourceFrom = cycle % apparentLength;
        // `cycle` will be an ever-increasing counter, so apply modulus

        if (sourceFrom < sourceLength) {
            // "Foobar" -> "bar     " || "r      F"

            // "bar"
            copySubstring(source, destination, sourceFrom,
                          min(static_cast<int>(sourceLength - sourceFrom), static_cast<int>(size)));
